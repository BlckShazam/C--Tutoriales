	...
	*/
	...
	void Start () {
		...
		// multiplicar "tablaDeMultiplicar" por el valor correspondiente.)
				while(contador<=10){
						int resultado = tablaDeMultiplicar * contador;
						Debug.Log (tablaDeMultiplicar+" * "+contador+" = "+resultado ); // otra forma --> Debug.Log (tablaDeMultiplicar+" * "+contador+" = "+(tablaDeMultiplicar * contador) );
						contador++;
	...
	*/
	...
	void Start () {
		...
		// "los elementos", "de un array":
				...
				string[] mensajes = {"Esto es", "un ejemplo", "de como", "se pueden", "recorrer todos", "los elementos", "de un array", "con contador"};
		...
		// array sera la numero 10-1=9 debido a que se empieza a contar desde 0)
				...
				while (cont < mensajes.Length) {
						Debug.Log (mensajes[cont++]); //primero se evalua cont y luego se le suma 1
